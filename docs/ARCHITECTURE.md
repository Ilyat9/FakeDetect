...
## Мульти-тенантность: чеклист «новый tenant-scoped эндпоинт» (F-C1)

Изоляция тенантов держится на двух независимых уровнях защиты — ни один из них
не единственная линия обороны:

1. **SQL-уровень**: запрос к БД сам фильтрует по `tenant_id` (`WHERE tenant_id = ?`),
   либо для агрегатов — через `_tenant_clause()` в `app/database.py`.
2. **Application-уровень**: для эндпоинтов вида "получить сущность по id" —
   после `SELECT ... WHERE id = ?` (без фильтра по tenant в самом запросе)
   результат обязан пройти через `tenancy.ensure_owned(entity, ctx, label=...)`
   (`app/services/tenancy.py`) — это и есть страховочная сетка на случай, если
   когда-нибудь забудут добавить `tenant_id` в SQL. Всегда 404, никогда 403 —
   403 сам по себе раскрывает факт существования чужого id.

При добавлении нового tenant-scoped эндпоинта, отдающего сущность по id:

- [ ] Резолвить контекст через `tenancy.require_ctx(request, min_role=...)`.
- [ ] Если запрос — «получить по id»: вызвать `tenancy.ensure_owned(entity, ctx, label=...)`
      сразу после фетча, до любой отдачи данных наружу. Смотри `app/routers/cases.py`,
      `app/routers/watches.py`, `app/routers/batch.py` как образец.
- [ ] Если запрос — листинг/агрегат: передать `tenant_id=ctx.tenant_id` в SQL-функцию
      напрямую (см. `app/database.py`).
- [ ] Добавить эндпоинт в таблицу `RESOURCE_ENDPOINTS` в
      `tests/test_tenant_isolation.py` (или отдельный тест рядом, если сетап не
      ложится в общий паттерн) — это единый источник правды для регрессии
      изоляции тенантов, туда же приходить при разборе будущих инцидентов.
