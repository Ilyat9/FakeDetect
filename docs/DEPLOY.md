...
## Клиентский / production-деплой (не публичное демо)

Если инстанс обслуживает реальных клиентов (не анонимное демо), обязательно задайте
в `.env`:

```
API_SECRET_KEY=<случайная строка>
STRICT_AUTH=1
```

`STRICT_AUTH=1` заставляет приложение отказаться стартовать, если `API_SECRET_KEY`
не задан, вместо тихого отката в open mode (все запросы = owner-права в Default
tenant). Без `STRICT_AUTH` забытый `API_SECRET_KEY` даёт warning в логах при
старте, но сервис всё равно поднимется в open mode — это осознанный компромисс
только для локальной разработки и публичного демо (см. F-C5 в
`docs/COMPROMISES.md`).
