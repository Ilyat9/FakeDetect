...
from core.config import MAX_IMAGE_UPLOAD_BYTES, get_api_key_for_provider, get_secret, settings
from database import cache_get_result, cache_put_result, get_tenant, save_check
...
async def _run_analysis(ctx, original_bytes: bytes, suspect_bytes: bytes,
    ...
    from core.deadline import Deadline, reset_deadline, set_deadline
    ...
    try:
        try:
            ...
            from database import enqueue_retry
...
async def partner_stats(request: Request):
    ...
    ctx = await tenancy.partner_rate_limit(request)
    from database import count_checks_this_month, get_stats
