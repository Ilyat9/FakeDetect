...
| ID | Компромисс | Почему | Влияние | План после MVP |
|---|---|---|---|---|
| F-C1 | Изоляция тенантов — фильтрация `tenant_id` в SQL-запросах, без Row-Level Security | SQLite не поддерживает RLS; фильтры централизованы в db-функциях | Один пропущенный WHERE = утечка между тенантами | Postgres RLS policies после миграции (A-C2) — защита на уровне СУБД |
...
| F-C5 | Open mode (без API_SECRET_KEY) остаётся доступным | Обратная совместимость фронтенда и локальной разработки | В продакшене забыть задать секрет = незакрытый API | Startup warning + опциональный strict-mode флаг STRICT_AUTH=1 |
